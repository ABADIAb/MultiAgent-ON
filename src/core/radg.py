"""Risk-Adaptive Decision Gates (RADGs) — physical risk decision function.

Implements the Physical RADG from Architecture V5, Phase 6.
...
Mathematical formulation (ProblemStatement_v5, Section 4.3):
  Given U_sem <= tau_sem (already cleared by the Semantic RADG):
...
Note: The "clarify" action is handled upstream by the Semantic RADG.
The Physical RADG only sees paths that already passed semantic validation.
...
def evaluate_radg(qot_results: list[dict]) -> str:
    """Evaluate the Physical RADG decision.

    Inspects the QoT feasibility results from Phase 5 and maps them
    to an action from the Physical RADG action space {approve, replan}.
