...
### 5. Diurnal Operational Shift Framework (Evaluation Justification)
- **Context:** Rather than evaluating intent translation in a static batch, the 120-intent benchmark corpus models an operational shift (diurnal workflow) in a carrier-grade optical network. Real-world operator workloads follow a non-homogeneous, stochastic arrival stream where routine provisioning demands (Class I Nominal) arrive alongside underspecified requests (Class II Ambiguous), requests violating physical reach or modulation limits (Class III Infeasible), and contradictory or malformed constraints (Class IV Adversarial).
- **Theoretical Grounding & Dilemmas:**
...
- **Action:** In Chapter 5 (Experimental Evaluation & Benchmarks) and the final defense presentation, structure the experimental narrative around this Diurnal Operational Shift simulation to mathematically and operationally justify the multi-baseline ablation results.
