---
title: "Weekly Report 2026-05-26"
date: 2026-05-26
tags: [report, weekly]
status: active
---
...
1. **Professor Consultation (Goal 1 — Completed):** Meeting held on May 19 with Prof. Zhang, Zheng, and Aryanaz. Both proposals were presented and discussed. Key feedback received:
   ...
   - **Prof. Qiaolun:** Requested a comparison table/slides summarizing the differences of our approach compared to existing agentic AI works in the literature.
...
2. **LangGraph Scaffolding (Goal 3 — Completed):** Built the core graph structure:
   ...
   - All tests passing (`test_state.py`, `test_graph.py`).
...
1. **SOTA & Literature Comparison:** Create the comparison table and 1-2 slides summarizing the differences of our MultiAgentON approach compared to existing agentic AI systems in the literature (Prof. Qiaolun's request).
...
4. **Pydantic Schemas:** Map the baseline JSON schemas into Python Pydantic classes for the Lightpath Agent.

---

## 5. Do You Need Support?

Write here: Need RESTConf API documentation or a JSON response dump from the controller to start writing real `GET` requests in `topology.py`. Follow up with Aryanaz/Zheng.

---

## 6. One-Sentence Summary

Held the professor consultation meeting, scaffolded the full LangGraph StateGraph with mock agents and passing tests, and aligned the V2 architecture with the SOTA comparison and intent examples requested by the professors.
