...
- **Relevance to MultiAgentON**: Confucius validates the core premise of our work — that multi-agent LLM systems can manage real networks at scale. Their DAG-based workflow modeling parallels our LangGraph `StateGraph`.
...
- **What We Can Reuse**: The hibernate-and-wake pattern is directly applicable to our future long-horizon operations (e.g., waiting for testbed responses). The validation framework concept aligns with our HITL approach.
...
- **Relevance to MultiAgentON**: These works from the SJTU group represent the **closest competitors** in the optical domain. They validate multi-agent LLM systems for optical network operations.
- **Key Difference**: AutoLight focuses on **operational lifecycle management** (wavelength add/drop, failure management, power optimization) — i.e., Day-1/Day-2 operations. Our thesis targets the [[ProblemStatement_20260427_Felipe_Abadia|**joint routing AND compute scheduling** problem]] (Day-0 + Day-1), which is a fundamentally different optimization problem. Their agents do not decompose user intents into compute+routing allocation tuples $A^* = (r^*, b^*, c^*)$.
- **Critical Gap We Fill**: Neither SJTU work explicitly addresses the **intent-to-configuration translation** problem with HITL validation. Their agents receive structured inputs, not natural language intents.
...
| **Compute Scheduling** | ❌ | ❌ | ❌ | ❌ | ❌ | **✅ (planned)** |
| **Joint Routing + Compute** | ❌ | ❌ | ❌ | ❌ | ❌ | **✅ (thesis objective)** |
...
```
                        COMPUTE SCHEDULING
                              ↑
                              |
                    [MultiAgentON]  ← UNIQUE POSITION
                              |
         INTENT PARSING ------+------ QoT-AWARE ROUTING
                              |
                              |
                    PoliMi ←--+-→ AutoLight / SJTU
                 (NL→Config)       (Optical Ops)
                              |
                              ↓
                      Confucius / MARE
...
**No existing work combines ALL THREE**: (1) Natural language intent parsing, (2) QoT-aware optical routing, AND (3) compute scheduling within a single multi-agent LLM framework. This is our thesis's unique contribution.

### 3.2 Our Unique Selling Points (USPs)

1. **Joint Optimization Scope**: We are the first to frame the problem as $A^* = (r^*, b^*, c^*)$ — jointly optimizing routing, spectrum, AND compute node allocation through agentic coordination.
2. **Neurosymbolic Design Discipline**: While others use LLMs end-to-end (risking hallucinated physics), we enforce a strict "LLMs reason, tools calculate" separation. The QoT tool is a pure Python GN model, not an LLM approximation.
...
4. **HITL as Anti-GIGO**: Our `interrupt()`-based HITL validation is a formal safety mechanism, not an optional feature. This addresses the "confidently wrong" problem flagged by the SOTA as a critical research gap.
...
| **Compute scheduling agent not implemented** | This IS the thesis contribution. Must be designed and validated before defense. |
