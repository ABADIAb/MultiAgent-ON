...
> [!LAYOUT]
> A Venn diagram or intersection graphic showing three overlapping circles: NL Intent Parsing, QoT-Aware Routing, and Compute Scheduling. MultiAgentON is in the center.

- **The Missing Link:** Current research treats routing and compute as separate problems.
- **The Contribution:** We frame the problem as $A^* = (r^*, b^*, c^*)$.
- **The Outcome:** Jointly optimizing routing, spectrum, AND compute node allocation through agentic coordination.

![Image Suggestion: Venn diagram showing MultiAgentON at the intersection of Intent Parsing, QoT Routing, and Compute Scheduling]

<!-- Speaker Notes: This brings us to the core positioning gap. No existing work combines Natural Language intent parsing, QoT-aware optical routing, AND compute scheduling within a single multi-agent LLM framework. Our thesis fills this exact void, treating the network and compute resources as a joint optimization problem solved by specialized AI agents. -->
...
1. **Ideate Experiment 002:** Integrate the Routing Agent and the QoT tool into the current LangGraph MVP.
2. **Compute Scheduling Research:** Deep dive into how compute node allocation is modeled in optical networks to build our Compute Agent.
