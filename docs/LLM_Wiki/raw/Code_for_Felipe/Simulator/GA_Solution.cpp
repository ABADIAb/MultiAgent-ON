...
GA_Solution :: GA_Solution(vector<GeneCluster>& ch, bool permut)
{
	for(int i = 0; i < ch.size(); i ++)
		chromosome.push_back(std::make_unique<GeneCluster>(GeneCluster(ch[i])));
...
GA_Solution :: GA_Solution(vector<GeneCluster>& ch, vector<bool> genes, bool permut)
{
	for(int i = 0; i < ch.size(); i ++)
		chromosome.push_back(make_unique<GeneCluster>(GeneCluster(ch[i])));
...
GA_Solution :: GA_Solution(vector<GeneCluster>& ch, vector<int> genes, bool permut)
{
	for (int i = 0; i < ch.size(); i ++)
		chromosome.push_back(make_unique<GeneCluster>(GeneCluster(ch[i])));
...
GA_Solution :: GA_Solution(const GA_Solution& sol)
{
	for(int i = 0; i < sol.chromosome.size(); i ++)
		this -> chromosome.push_back(make_unique<GeneCluster>(GeneCluster(*sol.chromosome[i])));
...
GA_Solution :: GA_Solution(GA_Solution&& sol)
{
...
GA_Solution& GA_Solution::operator=(const GA_Solution& sol)
{
...
GA_Solution& GA_Solution::operator=(GA_Solution&& sol)
{
	this -> chromosome = move(sol.chromosome);
...
int GA_Solution :: print(ofstream& File, int print_weights) const
{
    for(int i = 0; i < chromosome.size(); i ++)
	{
...
namespace NS_NBGA
{
	int crossover_op(GA_Solution& one, GA_Solution& two, int pos)
	{
	...
	bool operator==(const GA_Solution& solution_one, const GA_Solution& solution_two)
	{
