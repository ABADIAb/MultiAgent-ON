...
// #define RATIO_OF_GENES_MUTATED_INIT 0.02
#define RATIO_OF_GENES_MUTATED_INIT 0.9

// defines the number of mutations that are higly probable by the end of the active state
// #define FINAL_SMALL_MUTATIONS 0.02
...
// #define RATIO_BEST_FIT_MARGIN_MIN 0.002
...
#define GEN_BEFORE_BEING_STUCK 20
...
// #define RATIO_OFFSPRING_STUCK 0.9
#define RATIO_OFFSPRING_ACTIVE 0.95
#define RATIO_OFFSPRING_STUCK 0.8
...
#define CROSSOVER_PROB_ACTIVE 0.5
#define CROSSOVER_PROB_STUCK_1 0.2
#define CROSSOVER_PROB_STUCK_2 0.2


#define LOG_FILE_NAME "NB_GA_log.txt"
...
namespace NS_NBGA
{
	...
	// Interfaces
		public:
			...
			enum Mode {OA_placement_single, OA_placement_multi, NB_placement};

			GeneticAlgorithm(int pop_size, vector<vector<float>>& chromosome_struct, vector<vector<int> > mutation_parts, 
								vector<vector<int> > important_mutation_parts, Mode mode, bool active_clusters = false, bool short_run = false,
								bool show_status = false, int max_number_gen = 0);
			~GeneticAlgorithm();

			int create_solution(vector<bool> genes, bool permutable, int ind) 
			{
				population[ind] = GA_Solution(chromosome_struct, genes, permutable);
				return 0; 
			}
			...
			{
				population[index] = GA_Solution(chromosome_struct, genes, permutable);
				return 0;
			}
			...
			int first_generation(bool init, vector<vector<int> > GA_initializer_vector);
			...
			vector<GA_Solution> get_best_solutions();
	...
	// General Variables
		...
		vector<GeneCluster> chromosome_struct;

		// each solution - a sequence of 0/1
		vector<GA_Solution> population;
		vector<GA_Solution> new_population;

		vector<GA_Solution> unique_solutions;
		...
		// maximum number of generation, it is determined as an input
	...
	// Variables that describe the process of looking for the optimum
		...
		unordered_set<GA_Solution, GA_Solution::HashFunction> best_solutions_feasible;
        unordered_set<GA_Solution, GA_Solution::HashFunction> best_solutions_unfeasible;
