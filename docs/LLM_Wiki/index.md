...
## Weekly Reports
- [[weekly_reports/Weekly_Report_20260929_Felipe_Abadia]]: Weekly report September 29, 2026. Architecture and implementation of modular comparative baselines (Proposed RADG, Always-On HITL, LLM-Only), segregated per-baseline telemetry, historical run version picker, automated cross-baseline Four Pillars visual suite, robust multi-stage $U_{sem}$ parsing, 5-minute intent watchdog, unified interactive/benchmark CLI, and 341 passing unit tests.
...
## Session Summaries
- [[session_summary/session_20260926_Radar_Metrics_Consistency_and_Evaluation_Hardening]]: Evaluation environment restructuring by model (`<baseline>/results/<LLM>/<timestamp>/` and comparative results in `tests/evaluation/results/<LLM>/<timestamp>/`), identifier sanitization (`sanitize_model_name`), visual asset consolidation (moving `gate_accuracy_matrix` to comparative suite and pruning per-baseline standalone charts), legacy run archiving, and streamlining `comparative_scalability_projection` into a single-panel plot comparing Proposed RADG vs. Always-On HITL operator interventions.
- [[session_summary/session_20260925_Evaluation_Hardening_MultiClass_Visuals_and_Benchmarks]]: Consolidated evaluation suite hardening across all 3 baselines (Proposed RADG, Always-On HITL, LLM-Only) on full (120) and compact (20) corpora: UAR physical invariant fix (0.0%), median-based telemetry migration eliminating latency skew, adversarial NaN literal preservation in PDDL parser forcing CFG fail-fast ($U_{sem}=1.0$), dynamic multi-class visual scaling, and git tracking cleanup.
- [[session_summary/session_20260924_LLM_Only_Calibrated_and_Results_Refactoring]]: LLM-Only ablation baseline calibration (blind forwarding $\mathcal{A}_{pre} = \{\text{approve}\}$, $FPR=100\%$, SDON controller runtime rejections on non-nominal traffic, GDA marked N/A, and Turn 1 wasted compute profiling), tailored visual analytics (`deployment_failure_matrix`, `wasted_compute_overhead`, `llm_only_ablation_dashboard`), telemetry exporter cleanup eliminating duplicate unversioned files in favor of timestamped files, pattern-based run resolution, workspace cleanup, and PNG/PDF gitignore rules.
- [[session_summary/session_20260923_Modular_Baselines_and_Evaluation_CLI]]: Modular baseline architecture (Proposed RADG, Always-On HITL, LLM-Only) in `tests/evaluation/baselines/`, segregated per-baseline telemetry, historical run picker, cross-baseline Four Pillars visual suite (Radar chart and Grouped Bar chart), unified interactive/benchmark CLI in `tests/evaluation/main.py`, Ctrl+C fix, legacy archiving, and 337 unit tests.
