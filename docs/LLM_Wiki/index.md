...
## Session Summaries
- [[session_summary/session_20260916_Nominal_Evaluation_Harness_and_Prompt_Hardening]]: Automated evaluation runner for Class I Nominal Intents on 17-node Nobel-Germany topology, diagnosis and resolution of `qwen2.5:3b` dummy zero-constraint filling and waypoint negative exclusion hallucinations, multi-node prompt and logic hardening (`pddl_parser.py`, `semantic_gate_node.py`, `reverse_prompt.py`, `intent_reconciler.py`), and baseline evaluation reporting.
