...
# Wiki Protocol: Rules and Workflows

This document defines the rules for managing the `docs/LLM_Wiki/` knowledge base. You must abide by these rules whenever interacting with the wiki.
...
- **Debrief Phase 1 (`/debrief1`)**: Session content planning routine. Execute the following steps:
  ...
  2. **Draft Recommendations**: Propose updates for the Weekly Report and the Issue Report (use the `report-generator` skill rules). Ask the user which items should be included.
...
- **Strict Linking Rule**: You MUST aggressively interlink concepts using `[[wikilinks]]`. If a concept exists, link to it. Do not rely solely on manual linking; the Deep Lint process enforces this.
