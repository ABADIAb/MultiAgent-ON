...
- **Post-Extraction Cleanup**: When processing a raw transcript into a final wiki document, you MUST perform a cleanup pass to optimize it for LLM context windows:
  - **Paragraph Consolidation**: Merge multiple disjointed `>` lines within a single speaking turn into a single, cohesive paragraph.
  - **Token and Line Reduction**: Drastically reduce token usage by removing filler words, stutters, and trivial speaking turns that don't add value.
  - **Technical Meaning Preservation**: Ensure that when merging lines and removing filler, the technical sense and understanding of what was said is fully preserved or even improved.
